...
function wigglyguildmiddle.Outlaw_Job_Clear(job)
	local player = CH('PLAYER')
	GAME:CutsceneMode(true)
	UI:ResetSpeaker()

	GROUND:TeleportTo(player, 436, 232, Direction.Up)
	GAME:MoveCamera(444, 232, 1, false)
	...
	if job.Client == 'magna' then
		...
		COMMON.MakeCharactersFromList({
			{'Magnemite_Left', 381, 221, Direction.Right},
			{'Magnemite_Right', 381, 251, Direction.Right},
			{'Magnezone', 436, 204, Direction.Down}
		})
		...
		local outlaw = RogueEssence.Ground.GroundChar(outlaw_monster, RogueElements.Loc(395, 232), Direction.Right, outlaw_monster.Species, 'Outlaw')
		outlaw:ReloadEvents()
		...
		local client = RogueEssence.Ground.GroundChar(client_monster, RogueElements.Loc(346, 204), Direction.Down, job.Client:gsub("^%l", string.upper), client_monster.Species)
		client:ReloadEvents()
...
function wigglyguildmiddle.Mission_Job_Clear(job)
	local player = CH('PLAYER')
	GAME:CutsceneMode(true)
	UI:ResetSpeaker()

	GROUND:TeleportTo(player, 244, 232, Direction.Up)
	GAME:MoveCamera(252, 224, 1, false)
	SOUND:StopBGM()
	...
	if job.Client == job.Target and job.Type ~= COMMON.MISSION_TYPE_ESCORT then
		...
		local client_monster = RogueEssence.Dungeon.MonsterID(job.Client, 0, "normal", client_gender)
		local client = RogueEssence.Ground.GroundChar(client_monster, RogueElements.Loc(244, 201), Direction.Down, job.Client:gsub("^%l", string.upper), client_monster.Species)
		client:ReloadEvents()
		GAME:GetCurrentGround():AddTempChar(client)
		...
		local client = RogueEssence.Ground.GroundChar(client_monster, RogueElements.Loc(232, 201), Direction.Down, job.Client:gsub("^%l", string.upper), client_monster.Species)
		client:ReloadEvents()
		GAME:GetCurrentGround():AddTempChar(client)
...
function wigglyguildmiddle.ChapterHideScript()
GROUND:Hide("Sunflora")
GROUND:Hide("Bidoof_roamer")
GROUND:Hide("crawdaunt_roamer")
GROUND:Hide("Chimecho")
