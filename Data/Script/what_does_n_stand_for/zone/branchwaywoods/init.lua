...
function branchwaywoodsboss.ExitSegment(zone, result, rescue, segmentID, mapID)
  ...
  SV.branchwaywoodsentrance.DungeonFailed = true
    ...
    if segmentID == 0 then
			...
			--GAME:EnterZone('branchwaywoods', -1, 1, 0) --only use for when you need to activate a cutscene after a Dungeon.
			COMMON.EndDungeonDay(result, 'branchwaywoods', -1, 1, 0) --works, but displays the Results Screen, don't want here.
